...
export default class WebVRM {
    ...
    public setBoneRotation(key: string, angle: THREE.Euler) {
    ...
    private attachMaterial(object3D: Object3D) {
        ...
        if (Array.isArray(object.material)) {
            object.material.forEach(
                (m, index) => (object.material[index] = createMaterial(m))
            );
        } else {
    ...
    private attachHumanoidBone() {
        for (const [
            i,
            bone
        ] of this.vrm.userData.gltfExtensions.VRM.humanoid.humanBones.entries()) {
